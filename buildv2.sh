...
if [ ! -f employees.sql ]; then
  wget -q https://raw.githubusercontent.com/datacharmer/test_db/master/load_titles.dump
fi
...
docker exec -i $docker_id mysql -uroot -p$PASSWORD employees < load_titles.dump
